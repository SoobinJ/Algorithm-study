...
- str : 가로별 지도를 해독한 문자열, 이를 answer에 업데이트
- b1, b2 : arr1, arr2의 값을 이진수화 한 후 n의 값보다 작을 경우 0으로 채워준 값
- sB1, sB2 : b1, b2값을 글자별로 잘라서 배열에 저장한 값
- 반복문을 통해서 n만큼 반복
- toString(2) : 이진수로 변환
- padStart(n,0) : 문자열의 길이가 n보다 작을 경우 0으로 채워줌
- split("") : 글자별로 잘라서 배열에 
- sB1, sB2값이 둘 다 '0'인경우 str에 공백을 추가해주고 다른 경우 str에 '#'추가해줌
- 배열의 길이만큼 str에 추가해 준 후 그 값을 answer[i]에 저장해줌
